...
describe("subscriptions-core lifecycle integration", () => {
  const previousStateDir = process.env.GUTU_STATE_DIR;
  let stateDir = "";

  beforeEach(() => {
    stateDir = mkdtempSync(join(tmpdir(), "subscriptions-core-"));
  ...
  it("creates, advances, and reconciles a governed business record", async () => {
    const created = await createPrimaryRecord({
      ...
      actorId: "actor_admin",
      recordId: "subscriptions-core:demo",
      title: "Subscriptions Core Demo",
      ...
      correlationId: "subscriptions-core:corr",
      processId: "subscriptions-lifecycle:demo"
    });
    ...
    const reconciled = await reconcilePrimaryRecord({
      ...
      actorId: "actor_admin",
      recordId: "subscriptions-core:demo",
      exceptionId: "subscriptions-core:exception",
    ...
    const amended = await amendPrimaryRecord({
      ...
      actorId: "actor_admin",
      recordId: "subscriptions-core:demo",
      amendedRecordId: "subscriptions-core:demo:amended",
      expectedRevisionNo: 5,
      title: "Subscriptions Core Demo Amendment",
      reasonCode: "commercial-correction"
    });
    expect(amended.amendedRecordId).toBe("subscriptions-core:demo:amended");

    const reversed = await reversePrimaryRecord({
      tenantId: "tenant_demo",
      actorId: "actor_admin",
      recordId: "subscriptions-core:demo:amended",
      reversalRecordId: "subscriptions-core:demo:amended:reversal",
      expectedRevisionNo: 1,
      reasonCode: "cancel-amendment"
    });
    expect(reversed.reversalRecordId).toBe("subscriptions-core:demo:amended:reversal");
