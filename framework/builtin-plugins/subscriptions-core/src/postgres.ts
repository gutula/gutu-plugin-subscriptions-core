export type SubscriptionsCoreSqlOptions = {
  schemaName?: string;
  dropSchema?: boolean;
};

export function buildSubscriptionsCoreMigrationSql(options: SubscriptionsCoreSqlOptions = {}): string[] {
  const schemaName = normalizeIdentifier(options.schemaName ?? "subscriptions_core", "schemaName");
  return [
    ...
    `CREATE UNIQUE INDEX IF NOT EXISTS ${getSubscriptionsCoreLookupIndexName()} ON ${schemaName}.primary_records (tenant_id, title, correlation_id);`,
    `CREATE INDEX IF NOT EXISTS ${getSubscriptionsCoreStatusIndexName()} ON ${schemaName}.exception_records (tenant_id, status, severity);`
  ];
}

export function buildSubscriptionsCoreRollbackSql(options: SubscriptionsCoreSqlOptions = {}): string[] {
  const schemaName = normalizeIdentifier(options.schemaName ?? "subscriptions_core", "schemaName");
  const dropSchema = options.dropSchema ?? schemaName !== "subscriptions_core";
  return [
...
export function getSubscriptionsCoreLookupIndexName(): string {
  return "subscriptions_core_primary_lookup_idx";
}

export function getSubscriptionsCoreStatusIndexName(): string {
  return "subscriptions_core_exception_status_idx";
}
