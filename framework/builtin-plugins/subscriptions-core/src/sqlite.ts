export type SubscriptionsCoreSqliteOptions = {
  tablePrefix?: string;
};

export function buildSubscriptionsCoreSqliteMigrationSql(options: SubscriptionsCoreSqliteOptions = {}): string[] {
  const tablePrefix = normalizePrefix(options.tablePrefix ?? "subscriptions_core_");
  return [
    ...
    `CREATE UNIQUE INDEX IF NOT EXISTS ${getSubscriptionsCoreSqliteLookupIndexName(tablePrefix)} ON ${tablePrefix}primary_records (tenant_id, title, correlation_id);`,
    `CREATE INDEX IF NOT EXISTS ${getSubscriptionsCoreSqliteStatusIndexName(tablePrefix)} ON ${tablePrefix}exception_records (tenant_id, status, severity);`
  ];
}

export function buildSubscriptionsCoreSqliteRollbackSql(options: SubscriptionsCoreSqliteOptions = {}): string[] {
  const tablePrefix = normalizePrefix(options.tablePrefix ?? "subscriptions_core_");
  return [
...
export function getSubscriptionsCoreSqliteLookupIndexName(tablePrefix = "subscriptions_core_"): string {
  return `${normalizePrefix(tablePrefix)}primary_lookup_idx`;
}

export function getSubscriptionsCoreSqliteStatusIndexName(tablePrefix = "subscriptions_core_"): string {
